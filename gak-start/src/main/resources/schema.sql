...
-- 站内消息先持久化，再通知在线连接；用户删除不级联清除发送历史。
CREATE TABLE IF NOT EXISTS gak_message (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    source VARCHAR(64) NOT NULL,
    idempotency_key VARCHAR(160) NOT NULL,
    payload_hash VARCHAR(64) NOT NULL,
    sender_id BIGINT,
    audience VARCHAR(16) NOT NULL CHECK (audience IN ('USERS','ALL')),
    category VARCHAR(24) NOT NULL CHECK (category IN ('ANNOUNCEMENT','BUSINESS','SECURITY')),
    priority VARCHAR(16) NOT NULL CHECK (priority IN ('NORMAL','IMPORTANT')),
    title VARCHAR(120) NOT NULL,
    body TEXT NOT NULL,
    target VARCHAR(32) CHECK (target IS NULL OR target='HOME'),
    created_at TIMESTAMP NOT NULL DEFAULT clock_timestamp()
);
CREATE UNIQUE INDEX IF NOT EXISTS uk_message_source_idempotency ON gak_message(source,idempotency_key);
CREATE INDEX IF NOT EXISTS idx_message_created ON gak_message(created_at DESC,id DESC);
CREATE TABLE IF NOT EXISTS gak_message_recipient (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    message_id BIGINT NOT NULL REFERENCES gak_message(id),
    user_id BIGINT NOT NULL,
    received_at TIMESTAMP NOT NULL DEFAULT clock_timestamp(),
    read_at TIMESTAMP
);
CREATE UNIQUE INDEX IF NOT EXISTS uk_message_recipient_message_user ON gak_message_recipient(message_id,user_id);
CREATE INDEX IF NOT EXISTS idx_message_recipient_user_received ON gak_message_recipient(user_id,received_at DESC,id DESC);
CREATE INDEX IF NOT EXISTS idx_message_recipient_user_unread ON gak_message_recipient(user_id,received_at DESC,id DESC) WHERE read_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_permission_audit_notification_revision ON gak_permission_audit_log(target_user_id) WHERE action_type='REPLACE_APPS';
